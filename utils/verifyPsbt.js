...
function hashPsbt(base64String, algorithm = "sha256") {
  return crypto
    ...
    .update(Buffer.from(removeTaprootInternalKey(base64String), "hex"))
...
function removeAllSignatures(psbtBase64) {
  ...
  psbt.data.inputs.forEach((input, index) => {
    ...
    if (input.finalScriptWitness) {
      delete input.finalScriptWitness; // Remove all finalScriptWitnes
    }
  });
...
function removeTaprootInternalKey(psbtBase64) {
  const psbt = bitcoin.Psbt.fromHex(psbtBase64);

  // Loop through all inputs and clear signatures
  psbt.data.inputs.forEach((input, index) => {
    if (input.tapInternalKey) {
      delete input.tapInternalKey; // Remove all taproot internal key
    }
  });
